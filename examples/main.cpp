...
html_node hoge()
{
    ...
    auto prop_list = css::make_property_list(props::property::background_color(css::palette::lime),
                                             props::property::color(css::palette::red));

    // do something
    int num = 10;

    return div_(aonyx::dom::attrs::style(prop_list))(div_("a"), div_("<p>hoge!!</p>"), div_("c"));
}

void top(AONYX_PARAM(req, res))
{
    auto node = html(attribute{"lang", "en"})(head(), body(div_(span("Hello, World!")), hoge()));
...
int main()
{
    ...
    // グローバルミドルウェア: ロギング
    server.use(
        [](AONYX_PARAM(req, res), aonyx::http::next_t next)
        {
    ...
    router.get("/top", top);
    router.get<int>("/users/{}",
                    [](AONYX_PARAM(req, res), int id)
                    {
                        res.body = aonyx::dom::div_(std::to_string(id)).to_string();
                        res.status = 200;
                        res.headers["Content-Type"] = "text/html";
