...
bool call_handler(Fn fn,
                  const http::request& req,
                  http::response& res,
                  const inner_handler_params_t& params,
                  std::index_sequence<I...>)
{
    call_handler_impl<Fn, std::tuple_element_t<I, handler_args_tuple_t<Fn>>...>(
        fn, req, res, (param_from_string<std::tuple_element_t<I, handler_args_tuple_t<Fn>>>(params[I]))...);
    return true;
}
...
bool call_handler(Fn fn, const http::request& req, http::response& res, const inner_handler_params_t& params)
{
    ...
    return call_handler(fn, req, res, params, std::make_index_sequence<handler_args_size_v<Fn>>{});
}
} // namespace details
} // namespace util
} // namespace aonyx
