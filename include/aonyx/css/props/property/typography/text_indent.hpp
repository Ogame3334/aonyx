...
struct text_indent : types::property_base<"text-indent">, constants::global_values<text_indent>
{
    ...
    text_indent(util::numerical v, bool hanging)
        : super_property(std::format("{} {}", v.to_string(), hanging ? "hanging" : ""))
    {
    }
    text_indent(util::numerical v, bool hanging, bool each_line)
        : super_property(std::format("{} {} {}", v.to_string(), hanging ? "hanging" : "", each_line ? "each-line" : ""))
    {
    }
};
} // namespace property
