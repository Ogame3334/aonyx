/** @brief CSS property: padding-inline-start
 *
 * padding-inline-start CSS property.
 */
#pragma once

#include <string>
#include <string_view>
#include <format>

#include <aonyx/css/props/constants/global_values.hpp>
#include <aonyx/css/props/types/property.hpp>
#include <aonyx/util/numerical.hpp>

namespace aonyx
{
namespace css
{
namespace props
{
namespace property
{
struct padding_inline_start : types::property_base<"padding-inline-start">,
                              constants::global_values<padding_inline_start>
{
    using super_property = types::property_base<"padding-inline-start">;

    padding_inline_start() = delete;
    padding_inline_start(const std::string_view s) : super_property(std::string(s))
    {
    }
    padding_inline_start(util::numerical s) : super_property(s.to_string())
    {
    }
};
} // namespace property
} // namespace props
} // namespace css
} // namespace aonyx
